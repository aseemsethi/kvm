...
// The value of the setting in the array comes from the ref below.
// Ref: APPENDIX B -  FIELD ENCODING IN VMCS
// The VMCS data are organized into six logical groups:
// Guest-state area. 
// 		Processor state is saved into the guest-state area on VM exits and 
// 		loaded from there on VM entries.
// Host-state area. 
// 		Processor state is loaded from the host-state area on VM exits.
...
// 		These fields receive information on VM exits and describe the 
// 		cause and the nature of VM exits. On some processors, these 
